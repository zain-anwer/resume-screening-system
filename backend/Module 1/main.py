...
from modules.parser import get_all_candidates, extract_text, extract_cnic_details
from modules.cleaner import clean_text

OUTPUT_DIR = Path("output")
OUTPUT_DIR.mkdir(exist_ok=True)

candidates = get_all_candidates("jobs")
...
def process_candidates():
    ...
    for candidate in candidates:
        ...
        if candidate["resume"] is not None:
            ...
            if isinstance(resume_result, dict):
                resume_text = clean_text(resume_result.get("text", ""))
                resume_manual_review = bool(resume_result.get("manual_review", False))
                resume_blurred = bool(resume_result.get("blurred", False))
                resume_blur_score = float(resume_result.get("blur_score", 0.0))
            else:
        ...
        if candidate["cnic"] is not None:
            ...
            if isinstance(cnic_result, dict):
                cnic_manual_review = bool(cnic_result.get("blurred", False))
                cnic_blurred = bool(cnic_result.get("blurred", False))
                cnic_blur_score = float(cnic_result.get("blur_score", 0.0))

        cleaned_resume_text = clean_text(resume_text)
        cleaned_cnic_name = clean_text(cnic_data.get("name", ""))
        cleaned_cnic_dob = clean_text(cnic_data.get("dob", ""))
        cleaned_cnic_number = clean_text(cnic_data.get("cnic", ""))

        record = {
            "hash_id": hashlib.sha256(
                f"{job_category}|{candidate_id}|{cleaned_resume_text}".encode("utf-8")
            ).hexdigest(),
    ...
    output_path = OUTPUT_DIR / "all_candidates.json"
    with open(output_path, "w", encoding="utf-8") as output_file:
        json.dump(all_candidates, output_file, ensure_ascii=False, indent=2)
